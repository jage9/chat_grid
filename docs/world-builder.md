...
Inside a region its sound is centered. Outside, the nearest point on the rectangle determines the sound direction; volume follows a strong squared fade to silence at the configured Fade distance. With the default three-square distance, one square outside plays at about 44 percent of the configured volume, two squares at 11 percent, and three squares is silent. Gain and direction use the same 0.2-second smoothing as other positional sounds, including a fade to silence before an out-of-range loop is released. Ambiances use the World audio layer, master volume, shared standard/HRTF renderer, and wall/acoustic-zone transmission. The rectangles are lightly shaded on the visual grid.
