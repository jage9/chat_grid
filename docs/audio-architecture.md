...
- An elevator's optional `emitSound` remains a normal object emitter rather than following the car. A rider hears it through normal item distance and floor rules while the door is open; the client suppresses it only while that rider is inside with the door closed.
