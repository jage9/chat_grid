...
- The elevator object's own `z` remains its canonical item position while rider and car travel heights change separately. Its optional emitter uses normal item floor rules while the door is open and is suppressed for an inside rider only while the door is closed.
